...
def sacd(env_fn,
    ...
    def get_action(o, deterministic=False):
        logits = main.policy(torch.Tensor(o.reshape(1,-1))).squeeze()
        if deterministic:
            action = torch.argmax(logits).data.numpy().item()
        else:
            m = Categorical(logits=logits)
            action = m.sample().data.numpy().item()
    ...
    for t in range(total_steps):
        ...
        if d or (ep_len == max_ep_len):
            ...
            for _ in range(ep_len):
                ...
                q1, q2, logp_a, q1_pi,q2_pi, logp_pi,entropy, v = main(obs1, acts)
                ...
                v_backup = (min_q_pi + alpha * entropy).detach()

                advantage = (q_backup - v).detach()

                # Soft actor-critic losses
                pi_loss = torch.mean(-alpha * entropy - advantage * logp_a)
...
if __name__ == '__main__':
    ...
    sacd(lambda : gym.make(args.env), actor_critic=core.ActorCritic,
